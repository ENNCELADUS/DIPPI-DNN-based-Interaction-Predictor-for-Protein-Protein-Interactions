...
#SBATCH --output=logs/v6/slurm_%j.out
#SBATCH --error=logs/v6/slurm_%j.err
...
python -m src.data_preprocess.prepare_tppni_datasets --config configs/v6.yaml

torchrun --standalone --nproc_per_node="${NGPUS}" -m src.run -- --config configs/v6.yaml
