...
Tests the complete pretrain→finetune→evaluate workflow using minimal
...
from src.utils.config import extract_keys, load_config
...
# Artifact paths
LOGS_DIR = Path("logs/v3")
MODELS_DIR = Path("models/v3")
CONFIG_PATH = Path("configs/test_e2e.yaml")
...
def test_e2e_full_pipeline(cleanup_after_tests):
    """
    Test full_pipeline mode: pretrain → finetune.
    ...
    """
    print("\n" + "=" * 80)
    print("TEST: E2E Full Pipeline (pretrain → finetune)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("✓ TEST PASSED: E2E Full Pipeline")
    print("=" * 80)


def test_e2e_eval_only(cleanup_after_tests):
    """
    Test eval_only mode: evaluate using finetune checkpoint.
    ...
    Note: Depends on test_e2e_full_pipeline to have run first.
    """
    print("\n" + "=" * 80)
    print("TEST: E2E Eval Only")
    print("=" * 80)
    ...
    assert finetune_checkpoint.exists(), (
        ...
        "Ensure test_e2e_full_pipeline ran first."
    )
    ...
    eval_config_path = CONFIG_PATH.parent / "test_e2e_eval_temp.yaml"
...
def test_v3_config_all_keys_used():
    """
    Test that all keys in test_e2e.yaml config are actually used.

    This catches typos and ensures no unnecessary config keys remain.
    """
    print("\n" + "=" * 80)
    print("TEST: V3 Config Key Usage Validation")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("✓ TEST PASSED: V3 Config Key Usage Validation")
    print("=" * 80)


# --------------------------------------------------------------------------- #
# Autocast integration coverage
# --------------------------------------------------------------------------- #


class _TinyModel(nn.Module):
    """Minimal model for trainer-level tests."""
    ...
    def forward(self, batch):
        x = batch.get("x", torch.zeros(1, 1, device=self.linear.weight.device))
        return {"loss": self.linear(x).mean()}


def _build_toy_trainer(device: torch.device) -> Trainer:
    return Trainer(
        model=_TinyModel(),
        device=device,
        optimizer_cfg={"type": "sgd", "lr": 0.01},
        amp_cfg={"enabled": True, "dtype": "bf16"},
    )


def test_trainer_autocast_prefers_torch_amp(monkeypatch):
    device = torch.device("cpu")
    trainer = _build_toy_trainer(device)

    called = {}

    def fake_autocast(*, device_type=None, dtype=None):
    ...
    assert called["device_type"] == device.type
    assert called["dtype"] == trainer.amp_dtype


def test_trainer_autocast_falls_back_to_cuda_amp(monkeypatch):
    if not hasattr(torch.cuda, "amp") or not hasattr(torch.cuda.amp, "autocast"):
        pytest.skip("torch.cuda.amp.autocast not available")
    ...
    def fake_cuda_autocast(*, dtype=None):
        called["dtype"] = dtype
        return contextlib.nullcontext()

    monkeypatch.delattr(torch, "amp", raising=False)
    monkeypatch.setattr(torch.cuda.amp, "autocast", fake_cuda_autocast)
