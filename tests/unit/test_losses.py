...
def test_binary_classification_loss_supports_label_smoothing() -> None:
    ...
    config = LossConfig(
        loss_type="bce_with_logits", pos_weight=1.0, label_smoothing=0.2
    )
...
def test_binary_classification_loss_supports_pos_weight() -> None:
    ...
    weighted_config = LossConfig(
        loss_type="bce_with_logits", pos_weight=2.0, label_smoothing=0.0
    )
    base_loss = binary_classification_loss(
        logits=logits, labels=labels, loss_config=base_config
    )
