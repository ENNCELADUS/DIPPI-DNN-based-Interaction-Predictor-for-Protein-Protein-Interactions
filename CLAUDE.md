...
- Always run Python-related commands inside the `esm` conda environment for this repository.
- Activate with: `source /opt/homebrew/Caskroom/miniconda/base/etc/profile.d/conda.sh && conda activate esm`
- Prefix every Bash tool call that runs Python, pytest, ruff, mypy, or any script with the command above.
- If `esm` is not active, stop and activate it before continuing.
...
- **Environment (required before Python commands)**: `conda activate esm`
