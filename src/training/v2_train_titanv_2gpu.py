...
def train_model(config):
    ...
    save_dir = f"models/v5_2_titanv_2gpu_training_{timestamp}"
    ...
    if torch.cuda.device_count() > 1 and config.get('multi_gpu', True):
        try:
            print(f"🚀 Attempting to use {torch.cuda.device_count()} TITAN V GPUs for training!")
...
def main():
    ...
    config = {
        ...
        'num_epochs': 50,
        'batch_size': 32,  # 2x TITAN V (24GB total VRAM) - 16 per GPU
        'learning_rate': 5e-5,
        'weight_decay': 1e-4,
        'patience': 10,
        ...
        # Multi-GPU settings
        'multi_gpu': True,  # Enabled for 2x TITAN V setup
