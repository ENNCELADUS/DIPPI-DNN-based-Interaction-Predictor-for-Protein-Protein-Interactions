#!/usr/bin/env python3
"""
Training script for Simple Concatenation PPI Classifier
Uses pretrained v2 MAE compressed representations (B, 960) concatenated to (B, 1920)
"""
...
# Import the simple concatenation model
from src.model.v2_1 import PPIClassifier_SimpleConcat, create_ppi_classifier_simple_concat, count_parameters
...
def train_model(config):
    ...
    print("🧬 SIMPLE CONCATENATION PPI CLASSIFIER TRAINING")
    print("=" * 60)
    ...
    save_dir = f"models/simple_concat_training_{timestamp}"
    ...
    model = create_ppi_classifier_simple_concat(config['v2_mae_path'])
    ...
    if torch.cuda.device_count() > 1 and config.get('multi_gpu', True):
        try:
            ...
            print(f"✅ Simple Concat model wrapped with DataParallel for multi-GPU training")
...
def main():
    ...
    config = {
        ...
        'num_epochs': 50,
        'batch_size': 256,  # 4x RTX 2080 Ti (64 per GPU)
        'learning_rate': 1e-4,  # Slightly higher LR for simple MLP
        'weight_decay': 1e-4,
        'patience': 15,  # More patience since simpler model
        
        # Data loading
        'num_workers': 0,  # Set to 0 to avoid system memory allocation issues
        ...
        # Multi-GPU settings
        'multi_gpu': True,
        'sync_batchnorm': False,  # Simple concat model doesn't need synchronized batchnorm
    }
